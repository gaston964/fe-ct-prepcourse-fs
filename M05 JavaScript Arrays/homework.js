...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   let suma = arrayOfNums.reduce((acc, num) => {
      return acc + num;
   })
   return suma;
}
...
function promedioResultadosTest(resultadosTest) {
   ...
   // Iterar a través de los elementos y sumarlos.
...
function todosIguales(array) {
   ...
   // Tu código:
   // Si el arreglo tiene 0 o 1 elemento, se consideran iguales por definición.
   if (array.length <= 1) {
      return true;
   }
   // Guardar el primer elemento para comparar con los demás.
   const primerElemento = array[0];
   // Iterar a través del arreglo, comenzando desde el segundo elemento.
   for (let i = 1; i < array.length; i++) {
      // Si encontramos un elemento diferente al primer elemento, retornar false.
      if (array[i] !== primerElemento) {
         return false;
      }
   }
   // Si llegamos hasta aquí, todos los elementos son iguales, retornar true.
   return true;
}
